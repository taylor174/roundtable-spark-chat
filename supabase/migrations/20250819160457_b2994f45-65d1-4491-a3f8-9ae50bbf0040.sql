-- Improve the advance_phase_atomic_v2 function to handle winner determination more robustly
-- This fixes the issue where automatic advancement fails due to null winner text
...
BEGIN
  ...
  IF v_round_status = 'vote' THEN
    ...
    -- If not a tie and we have a clear winner, advance to next round automatically
    IF NOT v_is_tie AND v_max_votes > 0 AND v_winner_text IS NOT NULL THEN
      -- Create next round in suggest phase
      ...
      VALUES (
        ...
        'suggest',
        NOW(),
        NOW() + MAKE_INTERVAL(secs => v_default_suggest_sec)
      )
    ...
    ELSE
      -- Just complete the round (tie or special case)
      UPDATE tables SET updated_at = NOW() WHERE id = p_table_id;
