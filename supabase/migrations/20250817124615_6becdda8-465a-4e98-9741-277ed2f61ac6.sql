-- Clean up duplicate blocks, keeping the latest one for each table_id, round_id
DELETE FROM blocks 
WHERE id NOT IN (
  SELECT DISTINCT ON (table_id, round_id) id
  FROM blocks 
  ORDER BY table_id, round_id, created_at DESC
);
...
-- Ensure blocks table has proper replica identity for realtime
